...
# renders a subagent's transcript). Everything here is width-parameterised (no
# module-level WIDTH) so it imports cleanly. This module is the PRIMITIVES home:
# width math, palette/SGR, ANSI strip/wrap, the security-critical neutralize(),
# inline markdown. The bash/python tokenizer + pretty-printer moved to
# core/codefmt.py (which imports this module, one direction).
import os
import re
import sys
import unicodedata
...
# --- code pretty-printing / highlighting -----------------------------------------
...
def format_code(code):
    ...
    return codefmt.format_code(code)


def render(code, width, ind="  "):
    from core import codefmt
    return codefmt.render(code, width, ind)
