# dashboard/markdown.py — a small, safe Markdown -> HTML subset.
#
# md_html + its inline/fence/table helpers, for the memory-note render and the
# assistant message bubbles. Escapes through ansi.escape_html; emits only the markup it
# writes itself.
import re
...
def md_html(text):
    ...
    try:
        ...
        return "<p>%s</p>" % escape_html(text or "").replace("\n", "<br>")
