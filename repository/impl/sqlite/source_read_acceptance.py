...
from repository.impl.sqlite import processing_runtime, scope_relations
...
def validate_source_read(connection: sqlite3.Connection, proposal: SourceReadProposal) -> None:
    ...
    expected = _expected_settings(connection, package.selection.settings, context.binding.scope)
...
def _expected_settings(
    connection: sqlite3.Connection, settings: RuntimeSettings, scope: ExtensionScope,
) -> EncodedDocument | None:
    return settings.for_scope(scope, scope_relations.related_scopes(connection, scope))
