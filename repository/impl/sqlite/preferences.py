...
class SqliteNewSessionRepository(NewSessionRepository):
    ...
    def save_preferences(self, preferences: NewSessionPreferences) -> None:
        with self.database.write() as connection:
            connection.execute(
    ...
    def save_draft(self, draft: NewSessionDraft, keep_newest: int) -> DraftWrite:
        with self.database.write() as connection:
            current = connection.execute(
                "SELECT * FROM new_session_drafts WHERE working_directory=?",
                (draft.working_directory,),
            ).fetchone()
            if current is not None and draft.sequence < current["sequence"]:
            ...
            connection.execute(
                ...
                (draft.working_directory, draft.text, draft.sequence),
            )
            connection.execute(
...
class SqlitePushSubscriptionRepository(PushSubscriptionRepository):
    ...
    def upsert(self, subscription: PushSubscription) -> None:
        with self.database.write() as connection:
            connection.execute(
                ...
                mapper.push_subscription_values(subscription),
            )
...
class SqlitePushSigningKeyRepository(PushSigningKeyRepository):
    ...
    def save_keypair(self, keypair: PushSigningKeypair) -> None:
        with self.database.write() as connection:
            connection.execute(
                ...
                (keypair.private_key_pem, keypair.public_key),
            )
