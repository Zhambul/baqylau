Feature: a lead waits for a foreground subagent

  Scenario Outline: the work a foreground subagent does is attributed to that subagent
    Given session configuration "primary" uses <harness> with model <model> and low effort
    When I launch session "primary" and assign work "ticker work" to the foreground subagent with prompt
      """
  ...
  Scenario Outline: a lead stays busy while a foreground subagent runs
    Given session configuration "primary" uses <harness> with model <model> and low effort
    When I launch session "primary" and assign work "color work" to the foreground subagent with prompt
      """
      Run the exact foreground shell command `sleep 20`. After it finishes,
      reply only with COLOR_WORK_DONE.
      """
    Then subagent work "color work" is running while its lead has status executing
