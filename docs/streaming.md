...
- **`claude-cmd-fmt.py`** (the `PostToolUse` Bash hook) does the work — it no
  ...
  the live output directly under it). **Exception — a file-reading command is
  NOT streamed at all:** a foreground `sed`/`grep`/`cat`/`head`/`tail` of a
  source file (the `code` render kind), or a `sed`/`grep` **slice** of a markdown
  one (the `md` kind's read plane) — `tools.read_command`, gated by
  `CLAUDE_MIRROR_CMD_READ` plus each kind's own `CLAUDE_MIRROR_*` — collapses to a
  click-to-expand **Read one-liner** instead
  ([click-to-view.md](click-to-view.md), *File-reading commands render as a Read
  one-liner*); `claude-cmd-pre.py` skips its live tee/tailer for the same command
  so the two hooks agree. The content-render kinds below still apply to a read
  that ISN'T collapsed (a subagent's — the substream owns it; a whole-document
  `cat x.md`, which streams by design; or a run with
  `CLAUDE_MIRROR_CMD_READ=0`).
