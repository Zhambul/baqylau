...
| [rewrite-design.md](rewrite-design.md) | **PROPOSED** v2 rewrite design — one daemon over an `eventsourcing` spine: Intake + SessionLog applications, lineage identity, mapper policies (inference/closers/classification), folds/policies/reactors with declared storage shapes + branch classes, Rust edge (shim + in-shell reporter), pane-host terminal surfaces, FastAPI notification-ID API, alerting, cooperation/MCP, migration gates, tradeoff ledger |
