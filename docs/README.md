...
| [handover-recorder-interpreter.md](handover-recorder-interpreter.md) | Handover narrative for the 2026-08-15 redesign — the decision-by-decision design dialogue, every live-fire bug and its evidence-first fix, deployment state, known gaps |
| [http-api.md](http-api.md) | **CURRENT** — the daemon's HTTP layer (`api/`, FastAPI/uvicorn): the typed request models, the guard-as-dependency, async SSE, what deliberately kept its wire shape, and the rejected alternatives |
| [recorder-interpreter.md](recorder-interpreter.md) | **CURRENT** session/evidence flow — launch-time registration by wrappers, hooks as pure recorders, the one interpreter (pull → translate → react), evidence-derived resume positions, watch directives, and why each alternative failed |
| [canonical-harness-architecture.md](canonical-harness-architecture.md) | **PROPOSED** harness-neutral architecture — simple plugin contracts, canonical facts, strict Claude/Codex containment, transactional raw plus canonical audit, one-cursor SSE, unchanged UI, and a direct downtime deployment with no compatibility layer or rollback system |
| [codex_capabilities_coverage.md](codex_capabilities_coverage.md) | Living item-by-item coverage record for Codex transcript items, tools, actors, attention, lifecycle, application state, and deliberate raw-only evidence |
