...
| [sessionapi.md](sessionapi.md) | The read-side session-data API — presentation channel vs read model, the `streams`-table keystone, fork-aware `sid_chain`, the transcript parse/paint split + `plugins.agent_usage()`, the fidelity ladder, why not an events table |
...
| [rewrite-design.md](rewrite-design.md) | **SUPERSEDED by rewrite-design-v3-claude.md** — the v2 event-sourced proposal (one daemon over an `eventsourcing` spine: Intake + SessionLog, folds/policies/reactors, Rust edge, notification-ID API); kept with `rewrite-design-review.md` (its 20-finding consolidated review) as the record of that design and the measured lessons |
| [rewrite-design-v3-claude.md](rewrite-design-v3-claude.md) | **PROPOSED** v3 rewrite design — the domain-engine architecture: why event sourcing was the wrong category (finding-by-finding), edge → per-session engines → tool-neutral domain model in SQLite → DTO API → surfaces/reactors, evidence-log audit (no replay, by ruling), open-facts-are-rows crash story, the tree as index + narrow discard rule, per-session revisions, streaming open-block + the 2026-08-03 measured write-granularity numbers (Claude TUI = per-API-round, codex = message, `claude -p` = ~2Hz token deltas), the TUI tap ladder, OTEL as daemon listener, future-goals mapping, invariants, migration sketch; §17 = the six-agent legacy gap audit (2026-08-03) + amendments — machine-scope services, the session-env contract, evidence degrade ladder + warning-light plane, display-order/amendment frames + agent scope as a subscription dimension, gesture verdicts + delivery-proof correlation, prepared-commit sync answer, per-kind finalize authority + ownership transfer, graces-as-evidence-rules, accounts/credential port + the relimit saga, audience facts + the slot allocator, Terminal-port identity ops, the web-local plane, usage temporality per source |
