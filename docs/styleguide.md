...
| Codex rollout record shapes (turn_context/event_msg/response_item/top-level grammar, exec-args decode, patch line counts, exit extraction, `usage_split`) | `plugins/codex/rollout.py` — `parse()`/`parse_line()`; presenters consume its TYPED records and never the raw grammar. There may be several (the codex stream Renderer paints the `event_msg` register; a dashboard conversation provider reads the `response_item` one — docs/codex.md *Two registers*): the grep test `test_renderer_consumes_the_parser` pins ONE GRAMMAR, not one consumer |
