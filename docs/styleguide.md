...
| The DIRECTION word on an agent's mail chip (`✉ from <peer>` vs `✉ to <peer>`) | `core/streamfmt.MAIL_FROM`/`MAIL_TO` — load-bearing on the web, not decoration: an INCOMING message is also a transcript `teammsg` record, so agent scope drops the op and keeps the bubble, while an OUTGOING one has no record at all and must be kept (`actclass.prose_block`) |
