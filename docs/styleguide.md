...
| GLOBAL (cross-session/cross-device) dashboard preferences store — the new-session form's `{cwd, model, effort}` | `dashboard/prefs.py` (kv table at `core.paths.DASH_PREFS_DB`, ~/.claude, durable) — the one dashboard store that is NOT per-session and CREATES its DB on demand (no session-alive meaning); `GET`/`POST /api/ns-prefs` are its only callers |
