...
| A block header's WEB-FACING wording (the op's `note`: `⏺ Agent "…" launched / finished · 21m 31s`) | `core/ops.py` owns the FIELD (`label(note=)`, threaded by `core/streamfmt.chip`); the WORDING is the producer's — `substream_render.agent_note` for the two subagent blocks, one builder so launch and finish cannot drift. The presenter renders it and never re-words a chip: parsing a chip apart to reword it is the sniffing `actclass` ended |
...
| Agent-team MAIL in the mirror: the `●` delivered / `◉ read · ` glyphs, their yellow/green colours, and the arrival/read ops themselves | `plugins/claude_code/msgs.py` — `GLYPH_NEW`/`GLYPH_READ`/`READ_PREFIX`/`MSG_*_RGB` + `event_ops(events, log)`, which BUILDS the ops — an arrival's chip and summary body share a copy-group, so the body is inside the block like any other block's body; the plugin's `census()` returns them, so the tool-agnostic `claude-scorebar.py` only emits what it is handed (it may not import a plugin's internals, and an entry script cannot be imported by the classifier that reads the glyphs back) |
