...
| Per-account usage read model: freshest-per-slug `usage`/`limit-hit` aggregation (the hit filed under its OWN stamped slug — a migrated session's DB carries the old account's stamp) + what-counts-as-a-window / rolled-over-window / effective-5h / limit-still-active arithmetic | `core/sessionapi.py` — `account_usage()`/`usage_windows()`/`window_span()`/`_window_rolled()`/`effective_five_hour()`/`effective_usage()`/`limit_hit_active()`; the dashboard serves the computed numbers (effective `usage`, `five_hour_eff`, `limit_hit`) and app.js only reads them, enumerating windows in the served order (docs/relimit.md) |
