...
| Codex run identity in the read model (`codex_aid` — the streams src_path basename, extension stripped) | `core/sessionapi.py` — `codex_aid()`/`codex_runs()`; the codex activity provider resolves ids only through them. `codex_runs()` also EXCLUDES a standalone host's OWN run (the run whose rollout IS the session's `transcript_path`), so its unstamped ops don't list as a scoped agent whose scope resolves to zero ops |
