...
| Memory-wiki vocabulary: the root path (`~/wiki/01`), the memory-op test, the mirror 🧠 `MARK`, the `memory` kv snapshot (write side), and the vault link-resolve/backlink/read helpers | `plugins/claude_code/memory.py` (`root()`/`is_memory`/`MARK`/`record`/`resolve`/`backlinks`/`read_note`; `file_fmt` + `substream_render` write it, the dashboard reads the kv + renders notes via `dashboard/notehtml.py`, docs/dashboard.md *Memory tab*) |
