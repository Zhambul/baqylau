...
| Live-or-parked state-DB path for a session: the SID form (falsy when neither file exists — "no session state") and the ROW form (always a usable path for a caller that only stats/reads it) | `core/sessionapi.py` — `state_db_for(sid)` / `session_db(row)`, deliberately two functions (docs/sessionapi.md). An inline `P.state_db(...) if isfile else P.parked_db(...)` at a call site is the bug they exist to prevent |
| A control-plane POST's audit target — the `(row, log, sdb)` triple every session-scoped handler files its `state_files`/`errors` rows under, incl. BOTH fallbacks for a sid with no audit row | `dashboard/http/base._Base._audit_target` — 11 handlers were re-encoding it in four spellings, one of which (`P.mirror_log("")` for a log-less upload) silently resolved to the dashboard process's own CWD SLUG and filed rows in an unrelated session's timeline. `get_copy`/`get_view` keep their own two lines on purpose (they need the STRICT `state_db_for`, since they branch on its absence) |
