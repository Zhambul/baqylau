...
| Per-account usage read model: freshest-per-slug `usage`/`limit-hit` aggregation (the hit filed under its OWN stamped slug — a migrated session's DB carries the old account's stamp) + rolled-over-window / effective-5h / limit-still-active arithmetic | `core/sessionapi.py` — `account_usage()`/`_window_rolled()`/`effective_five_hour()`/`effective_usage()`/`limit_hit_active()`; the dashboard serves the computed numbers (effective `usage`, `five_hour_eff`, `limit_hit`) and app.js only reads them (docs/relimit.md) |
