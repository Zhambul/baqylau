...
| A block header's WEB-FACING wording (the op's `note`: `⏺ Agent "…" launched / finished · 21m 31s`, `⏺ Message team-lead → rev-ui-util · read`) | `core/ops.py` owns the FIELD (`label(note=)`, threaded by `core/streamfmt.chip`); the WORDING is the producer's — `substream_render.agent_note` for the two subagent blocks and `msgs.note_new`/`note_read` for team mail, one builder per pair so the two halves cannot drift. The presenter renders it and never re-words a chip: parsing a chip apart to reword it is the sniffing `actclass` ended (its `legacy_note` fallback is for chips ALREADY ON DISK only) |
...
| Agent-team MAIL in the mirror: the `●` delivered / `◉ read · ` glyphs, their yellow/green colours, the web wording, the body cap, Claude Code's teammate LIFECYCLE-FRAME vocabulary (`idle_notification`/`task_assignment`/… — the JSON its inbox records carry instead of prose) and the arrival/read ops themselves | `plugins/claude_code/msgs.py` — `GLYPH_NEW`/`GLYPH_READ`/`READ_PREFIX`/`MSG_*_RGB`/`note_new`/`note_read`/`CAP_TEXT`/`FRAME_PHRASE`/`FRAME_TEXT`/`frame`/`frame_words` (one builder words a frame for BOTH surfaces, so the pane's chip and the web's note cannot disagree about the same frame) + `event_ops(events, log)`, which BUILDS the ops — an arrival's chip and its body (the MESSAGE, from the inbox record's `text`) share a copy-group, so the body is inside the block like any other block's body; the plugin's `census()` returns them, so the tool-agnostic `claude-scorebar.py` only emits what it is handed (it may not import a plugin's internals, and an entry script cannot be imported by the classifier that reads the glyphs back). `opshtml/actclass.legacy_mail_note` re-words pre-`note` chips through this owner, colour-gated like the classifier |
