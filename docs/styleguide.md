...
| The `+N more` fragment of a multi-file read one-liner (how many files one row stands for) | `core/streamfmt.py` `MORE_FILES` + `file_line(more=)` builds it, `dashboard/opshtml/actclass.readmore` parses it back to weight the web's collapsed summary ("Read 2 files"). Read off the TEXT, not carried as a second op field, for the same reason `diffstat` recovers `+A -R` that way: the terminal has to paint it, so the text is the one encoding |
