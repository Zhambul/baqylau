...
| WHO produced a block (the agent name a per-stream block leads with) | `core/ops.py` — the op's own `who` FIELD, never concatenated into its paint text. `core/streamfmt.chip` + the substream's file line set it; `bin/claude-mirror.py` `_who` composes it back at paint time (the pane is unchanged); the web dashboard's agent scope simply doesn't render it. Baked into `s` it forced the one surface that must NOT show it to parse an ANSI-coloured name back off a string — pinned by `test_the_terminal_composes_who_back_onto_the_line` |
