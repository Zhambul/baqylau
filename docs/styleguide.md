...
| Per-account usage read model: freshest-per-slug `usage`/`limit-hit` aggregation + effective-5h / limit-still-active arithmetic | `core/sessionapi.py` — `account_usage()`/`effective_five_hour()`/`limit_hit_active()`; the dashboard serves the computed numbers (`five_hour_eff`, `limit_hit`) and app.js only reads them (docs/relimit.md) |
