...
- **`~/.claude/settings.json`** — a `hooks` block. Every supported hook event
  ...
  **Exception — `WorktreeCreate`/`WorktreeRemove` must NOT be wired.** They are not
  ...
  | `UserPromptSubmit` | tab `thinking` + `ask_fmt` (clears stale `ask-pending`/`plan-pending` stashes — a new turn means any modal dialog is gone; dashboard.md › *Web ask* / *Web plan mode*) |
  | `PreToolUse`       | tab `pretool` (all tools) · `Task\|Agent` → `subagent_fmt.run_phase("push")` (stashes the Task description for the upcoming `SubagentStart`) · `Bash` → `cmd_pre` (rewrites the command to stream live — see [streaming.md](streaming.md) › *Live foreground streaming*; its `updatedInput` JSON is printed to the dispatcher's **stdout**, which is the one Claude Code reads) · `AskUserQuestion\|ExitPlanMode` → `ask_fmt` (stashes the pending dialog in the state DB kv — `ask-pending` questions / `plan-pending` plan markdown — the web ask/plan cards' source; dashboard.md › *Web ask* / *Web plan mode*) |
  | `PostToolUse` / `PostToolUseFailure` | tab `posttool` (all tools; ignored for an `agent_id` inner call) · `Bash` → `cmd_fmt` · `Read\|Edit\|Write\|MultiEdit\|NotebookEdit` → `file_fmt` · `Monitor` → `monitor_fmt` · `AskUserQuestion\|ExitPlanMode` → `ask_fmt` (a real answer/approval — clears the TOOL'S OWN pending key; the decline paths fire NO closing hook, so the turn-boundary clears below cover them) · `TaskCreate\|TaskUpdate` → `task_fmt` (re-snapshots the on-disk task dir into the `tasks` state-DB kv — the web tasks card's source; a `TaskUpdate` status flip fires no dedicated hook, so this is its only refresh signal; dashboard.md › *Web tasks*) · `SendMessage` → `mail_fmt` (team mail's MESSAGE row: `✉ <from> → <to>` + the text. The ONE formatter that deliberately does not skip an `agent_id` inner call — a teammate's send is most of team mail, and the send is the only moment its text exists; the inbox poller sees a fraction of it, dashboard.md › *Team mail*) · `Skill` → `skill_fmt` (one `✦ skill · <name>` block, worded `⏺ Skill(<name>)` on the web, with the invocation's ARGS behind the click — the tool_response carries no skill body, since Claude Code injects the loaded SKILL.md as a user-shaped turn rather than a tool result; dashboard.md › *Skills*) · **everything else** → `tool_fmt` (one quiet `· <name>(<request>)` line with the whole request AND the tool's answer behind the click — WebFetch/WebSearch/ToolSearch/Grep/Glob/an MCP tool/whatever ships next; dashboard.md › *Every other tool*). That last matcher is the **COMPLEMENT** of every matcher before it (`dispatch._GENERIC_TOOL`, a negative lookahead over `_ROUTED_TOOLS` — which also excludes `Task\|Agent`, routed on PreToolUse) rather than a list of names: the tools with a formatter are a closed set this table states, the tools without one are open-ended, and an allowlist's failure mode is SILENCE — which is what it was, a tool with no matcher rendering nothing at all. Adding a formatter for one of them removes it from the generic family by the same edit; both directions are ratcheted in tests/test_l1b_dispatch.py. **Failures fire `PostToolUseFailure`, not `PostToolUse`** — the dispatcher routes both identically, so a non-zero-exit command still reaches the mirror |
  ...
  | `TaskCreated` / `TaskCompleted` | `task_fmt` (`✚`/`✓ task #N · <subject>` to the mirror, + the same `tasks` kv snapshot as the `PostToolUse` route above) |
  | `PreCompact`       | tab `working` (compaction is busy with no tool/reply signal of its own — paint the busy magenta so the tab doesn't sit stale through it; `working`, not `thinking`, so no interrupt-watch is started) + `compact_fmt` (arms the `compacting` state-DB kv — the web ctx bar's collapse animation; magenta only says "busy", which compaction shares with every think, so the dashboard needs the latch to know it is specifically COMPACTING; dashboard.md › *Compaction on the ctx bar*) |
  | `PostCompact`      | `compact_fmt` (clears the latch). Previously UNROUTED (audit-subscriber only). It is the ONLY end-of-compaction signal — nothing else fires until the next turn, which can be minutes away — and its payload carries the `compact_summary` and the `trigger` (`manual` = you typed `/compact`, `auto` = the window filled) |
  | `PostToolBatch`    | `cmd_blocked` (a Bash call that RESOLVED WITHOUT EVER RUNNING). Previously UNROUTED (audit-subscriber only). Two outcomes end a Bash call with **no `PostToolUse` at all** — another `PreToolUse` hook DENIES it, or the permission prompt is REJECTED — and by then `cmd_pre` has already painted the `▶ foreground` header, claimed an fg slot, spawned a tailer and turned the tab BLUE. `PostToolBatch` fires once the batch resolves and carries EVERY call of it *with its `tool_response`*, so it is the only signal those two outcomes have. The handler tests for "never ran" **not by the response wording** (version-fragile, and there are two of them) but by the fg-live hand-off still being there: `cmd_fmt` consumes one per foreground command at `PostToolUse`, and by `PostToolBatch` every call in the batch has resolved — so a surviving record is a call whose `PostToolUse` never fired. It hands the orphaned tailer an honest `■ blocked · never ran` outcome and paints the tab `posttool` (WORKING), because the turn is still running. Left unrouted, the tailer timed out on writer-liveness and its `bg-recheck` painted the tab GREEN mid-turn — see [tab-colors.md](tab-colors.md) › *A Bash call that never ran* |
  ...
  | `StopFailure`      | tab `stop` (turn ended on an API error — keep the tab off the "busy" colour) + `stop_fmt` (fold whatever landed in the transcript; and when the payload carries an `agent_id` — a subagent that died on an API error, which fires no `SubagentStop` — finalise that agent's block/slot via `subagent_fmt.finalize`, else its streamer hangs and the tab stays blue) + `ask_fmt` (same turn-boundary clear as `Stop`) + `relimit` LAST (a main-session payload carrying `error="rate_limit"` = the account's limit blocked the turn — stamp the `limit-hit` kv and, when a fallback account has room, migrate the session to it via a `--resume` relaunch; docs/relimit.md) |
  ...
  **Why one dispatcher, and how behaviour is preserved:**
  ...
  - **Lazy handler imports.** Only the on-every-event subsystems (`adopt`,
    ...
    `monitor_fmt`/`stop_fmt`/`task_fmt`/`mail_fmt`/`subagent_fmt`/`split`) are imported
