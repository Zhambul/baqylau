...
The system is not ready for the requested product use. One phase (P05) is complete. There are 10 phases and 53 subtasks: 18 subtasks are done, 23 are in progress, and 12 are not started. These are task records, not a completion percentage.
...
| P02 — SDK and shared checks | in_progress | Installable SDKs, shared Python and web policy, the architecture checks, and the package template (`baqylau_dev new`) exist. P02-T01 to T05 are done. P02-T06 has the external artifact CI job (`release`) and a local pass of `make test-release-consumers`; a remote CI result needs a push by the user. |
| P03 — Runtime and cooperation | in_progress | Discovery, fixed package copies, private workers, registry, durable state, lifecycle controls, ordinary settings APIs, candidate settings and record migration, Keychain secret references, and related-scope settings exist. Health policy, removal notices, and job draining remain open. |
...
| P05 — Data and commands | done | Projections, typed records and queries, durable commands and observers, projection generations, candidate histories, view resets, settings and record migrations, Keychain secret references, and related-scope settings work. |
| P06 — Web and settings | in_progress | Settings → Extensions has catalog, lifecycle controls, confirmation, revision checks, failure display, and shutdown evidence. Package assets load from digest URLs. Package modules mount in session tabs, workspace pages, the toolbar, the status line, and the feed, and a feed view can replace one core entry kind. Each extension has a schema settings form, workspace overrides, secret controls, and custom panels on one save path. Each card shows recent operations and health; a failing extension is disabled by the host. Some browser checks for the feed slot, history switches, and stored entries after a disable remain open. |
| P07 — Kitty | not_started | No application extension pane or block renderer exists yet. |
| P08 — E2E and release | in_progress | The installable test kit starts a private host, reads its data, and signs off (P08-T01 done). The release runner and application conformance checks remain open. |
| P09 — Adapters | in_progress | The `baqylau-adapters` package exists. T01 (CLI contract) and T02 (detection and shell records) are done. T03–T06 have their repeatable work done: a session tab, a Kitty pane, and a pane read for each group; the feed rows; Slack threads; and the `git-activity` service with a consumer case. Open: the live session cases, a metrics chart, and the repository scope of the Git contract (a user decision). |
| P10 — Git | in_progress | P10-T01 is done: the separate `baqylau-git` package reads repository identity, status, history, trees, and diffs with no session. P10-T02 has the Kitty view and the web page with live refresh; its real Kitty and live two-session checks are open. P10-T03 is done: stage, unstage, and commit with a checked expected state. P10-T04 is done: draft generation with the host's model service. P10-T05 is done: push without force, typed failures, and reconciliation from the remote. Adapters cooperation (P10-T06) waits for P09. |
