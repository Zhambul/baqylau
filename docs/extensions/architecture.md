...
`SettingsOverrides` keeps explicit installation and exact non-installation scope values separate from manifest defaults. An absent installation override means inheritance, not a saved copy of the old default. `capture_settings()` in `extensions/models/settings.py` resolves the selected fallback without changing those raw choices. Package identity and effective settings checks are also in the model layer and are shared by the repository and registry. The repository imports no registry service or worker implementation. Settings management routes resolve installation defaults and exact scope overrides through the manager. Declared candidate settings migrations now run during enable or reload. Related workspace/session/repository scope resolution, secret references, and record migrations remain open.
...
This path is for ordinary schema-checked documents. Secret storage, secret resolution, custom forms, and related-scope precedence remain open. Do not place credential values in ordinary settings. The implementation does not detect or redact an arbitrary secret placed in an ordinary document. Settings changes do not reprocess history; P04/P05 must connect processing and rebuild semantics.
...
This implements settings migration only. Record migration, history heads, explicit history rollback, secret references, and related-scope inheritance remain open. Each conversion call has the normal worker deadline. Stop is checked between calls; there is no measured total migration-time limit yet.
