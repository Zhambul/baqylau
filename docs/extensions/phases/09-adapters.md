...
- Scope: the contract is session-scoped. A repository consumer lists the repository's sessions through the new host session list (see P10-T06), and reads the service for each session. This was the recommended option. The other option was a working directory in core shell facts, but harnesses do not report one for each command.
- Open:
  ...
  - The live case ran on recorded shells only. The CLI was not run against any system, so there is no live CLI version evidence.
