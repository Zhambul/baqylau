...
### A child agent's result card sits in the wrong place (after the parent's answer), or one child shows one result for two tasks

The web feed orders ops and conversation by TIMESTAMP, and a child's completion
can genuinely land after the parent's final answer (measured, session
`019fb66b-12a0`: answer 04:25:28.9, the child's `task_complete` 04:25:29.4). The
fix is semantic, and its whole evidence is inside the audited paint op:

- `SELECT op FROM ops WHERE session_id=? AND op LIKE '%ctask%'` — the child-task
  stamp (`core/childtask.py`): `{"id": "<agent>#<task>", "step": "start|end",
  "turn": "<parent turn>"}` on the `⇢ prompt` and `⇠ result` cards, header AND
  body. Read it against the symptom:
  - **no `ctask` at all** on a child's cards = pre-2026-07-31 history (nothing can
    re-stamp a parked op, and the ordering is then the old ts-only one), or a host
    adapter that never called `AgentStream.task()` — the parity contract
    (`tests/test_l1h_child_agent_parity.py`) fails on that, so suspect history
    first.
  - **`step:"end"` with no `turn`** = the task's parent turn is unknown, and the
    ordering rule is INERT by design. Correct and expected for every Claude
    agent (Claude Code records no turn id anywhere). For a CODEX child it means
    the replayed-parent prefix carried no `task_started` with a `turn_id` — an
    older codex, or a rollout whose prefix was trimmed differently.
  - **two `⇠ result` cards sharing one `id`** = a real bug (one task must paint
    one result); **two results with DIFFERENT ids** for one agent is correct — a
    follow-up task, or a re-tasked teammate's second streamer generation.
- The PARENT half is read-side and leaves no rows (like ctx saturation): whether
  the answer bubble is marked `final` comes from codex's `phase: "final_answer"`
  in the rollout named by `sessions.transcript_path`. If the stamps look right and
  the order is still wrong, read the rollout tail — an answer with no
  `final_answer` phase has no anchor to move.
...
- **Codex answered the WRONG option — "I typed my own answer and it picked the first choice".** The `web-answer` row says `ok:true` and there is no error at all, because the driver's steps all verified; the wrongness is only visible in the ROLLOUT. TELL: the rollout's `function_call_output` for the `request_user_input` call reads `answers: ["<the first option>", "user_note: <what the user actually typed>"]` while the `web-answer` row's card sent no selection. CAUSE (fixed 2026-07-31, docs/codex.md *The cursor is not the selection*): codex's `›` cursor selects nothing until a key takes it, and BOTH submitting keys do — ENTER *and* TAB (`tab to add notes`) — so a free-text answer that pressed tab from the cursor's resting place stapled the user's words onto option 1 as a note. The fix cursors onto codex's own appended `None of the above` row FIRST (`dialog.none_row`, found on SCREEN — it is not in the tool call, so no stash or kv shows it). REGRESSION TELLS: a `web-answer` `step:'noneof'` (the row was not on screen — a codex that stopped appending it; the gesture now FAILS rather than answering wrongly) or `step:'notes'` (tab never opened the note field, so the typed text would have gone to codex's composer). A codex DECLINE ("chat about this") writes the same row with `chat:true`; its own steps are `navigate` (RIGHT never reached the last question), `noneof`, `confirm` (the `Submit with unanswered questions?` step stayed up) and `close`. Pull the truth with `sql "SELECT ts, content FROM state_files WHERE session_id='<sid>' AND action='web-answer' ORDER BY ts"` and then read the rollout's matching `call_id` — the row alone cannot tell you WHICH option went in.
- **Tab flips green too early** — a `bg-recheck`/`bg-watch`/`notify` transition with
  ...
  (wrongly) concluded.
  **START with the canned anomaly `bg-recheck painted green mid-turn (the turn
  ran on after it)`** — it flags an applied `bg-recheck` green whose next
  turn-boundary-or-tool transition is a `pretool`/`posttool` instead of a
  `thinking`, i.e. the same turn kept running past the "your turn". A legitimate
  green is always followed by the next prompt, so a non-empty section IS the bug.
  Its shipped cause (session `674d78d1`, fixed 2026-07-31) is a **Bash call
  another `PreToolUse` hook DENIED** — or one whose permission prompt was
  REJECTED. Neither ever runs and neither fires `PostToolUse`, but
  `claude-cmd-pre.py` has already painted the header, teed the command, claimed
  the fg slot and turned the tab blue; with no `PostToolUse` the tailer gets no
  outcome, times out on writer-liveness and its `bg-recheck` reads the vanished
  writer as a manual CANCEL ("the turn is over") and paints green. The measured
  trace: `PreToolUse` 13:59:10 (`live fg stream: slot=2 …`), **no `PostToolUse`
  for that `tool_use_id` at all**, `PostToolBatch` 13:59:10, `state:fg-live`
  `remove-own` 13:59:12, `fg` stream `writer-gone` / `lines_emitted=0` 13:59:17,
  `bg-recheck(fg): no live markers remain` **applied** 13:59:16, next `pretool`
  13:59:27 — eleven seconds of "your turn" on a live session, and the same shape
  seven more times in a sibling session using the same project hook. THE
  DIAGNOSTIC: for the `tool_use_id` in the `PreToolUse` decision, look for its
  `PostToolUse` — its absence, next to a `PostToolBatch` at the same second, is
  conclusive; the transcript's `tool_result` for that id then names which refusal
  it was (`PreToolUse:Bash hook error: …` or `The user doesn't want to proceed
  with this tool use.`). On a CURRENT build `claude-cmd-blocked.py` runs on
  `PostToolBatch`, so the healthy trail is a `blocked before it ran: <tid> …`
...
- **A tool call (WebFetch / WebSearch / ToolSearch / Grep / an MCP tool) is missing
  from the mirror** — since 2026-07-31 EVERY tool with no formatter of its own gets a
  quiet `· <name>(<request>)` line with the request + the answer behind the click:
  `hook_events` where `handler='claude-tool-fmt.py'`, one row per call, its `decision`
  carrying `rendered: <name>(<request excerpt>)` plus ` +view` when the expansion was
  stashed. The routing is an EXCLUSION matcher (`dispatch._GENERIC_TOOL`), so a MISSING
  row is one of four things, in order of likelihood: (1) the tool has a formatter of its
  own and you are looking at the wrong handler (`claude-cmd-fmt.py`/`file-fmt`/`monitor`/
  `mail`/`skill`/`task`); (2) `ignored: subagent event (agent_id present)` — correct and
  expected, the substream renders that stream; (3) `ignored: no state DB (unhosted
  session)` — headless/daemon or already parked; (4) `ignored: quiet tool (TaskList)` —
  the deliberate skip list (`tool_fmt.SKIP`). A row that exists for the `subscriber`
  handler and NOT for `claude-tool-fmt.py` at the same `tool_use_id` means the step
  crashed: look in `errors` under `script='claude-tool-fmt.py'`. Two rows for one call —
  a formatter's AND the generic one's — means the exclusion list drifted from the routing
  table (the ratchet in tests/test_l1b_dispatch.py is what should have caught it). The
  expansion is a `view-stash` `state_files` row with `"kind": "tool"`; its absence with a
  `+view`-less decision means the payload had nothing to show (docs/dashboard.md *Every
  other tool*).
