...
- **Web tasks card missing / stale / wrong statuses** *(web tasks, since
  ...
  **A card frozen on a DEAD list after a `--resume` — every `tasks` write
  ...
  holding the event's own task (sid dir first, then the pinned drift dir, then
  a newest-mtime scan) and PINS a scan hit as a `tasks-dir`
  `{"action":"pin", "dir", "sid_dir", "task_id", "subject"}` state_files row +
  the `tasks-dir` kv — so a drifted session shows exactly one pin row and its
  later snapshots track the pinned dir; a frozen card WITHOUT a pin row on a
