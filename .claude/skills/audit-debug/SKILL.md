...
- **A `.md`/`.json`/`.yml` file rendered raw (or garbled) instead of pretty** —
  content render mode (markdown: `cat`/`head`/`tail` of a `.md`; JSON: `cat` of a
  `.json`; YAML: `cat`/`head`/`tail` of a `.yml`/`.yaml`; README § Command mirror
  pane). First confirm it was even engaged: the cmd-pre `hook_events` decision ends
  in `[md-render]`/`[json-render]`/`[yaml-render]`, and a `state_files`
  `render:<taskid>` `start` row exists (content `kind`) — if neither, the
  `*_source` detector didn't match (piped/redirected/chained command, a
  non-allowlisted tool like `bat`/`glow`/`jq`/`yq`, `head`/`tail` of a `.json` —
  JSON needs the whole file — or `CLAUDE_MIRROR_MD`/`_JSON`/`_YAML=0`), so raw is
  expected. If engaged but the
