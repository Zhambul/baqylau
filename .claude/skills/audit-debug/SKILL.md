...
- **A web attachment/screenshot "didn't attach" or the message went out without
  ...
  dashboard one.
  An upload row with `ok: false, why: "empty file"` is its own shape: the
  browser was handed a **zero-byte file PROMISE**, not bytes — copying a file
  out of an IDE (IntelliJ's Project-view *Copy*) puts a promise + the file's
  PATH on the clipboard, and the pre-2026-07-25 composer uploaded the husk. The
  fix makes the promise paste the PATH instead (kitty's behavior, *Web
  attachments* → *Promise-only files*), so on fixed code that row is GONE and
  the evidence moves to the frontend channel: a `web-client` row with `ev:
  attach.promise-paste`/`attach.promise-drop`, `n` = promises seen, `chars` =
  the path text pasted. A NEW `ok:false, why:"empty file"` row means the client
  is running STALE JS (cross-check the same device's `boot` row) — or the user
  really picked a 0-byte file, which the picker now refuses client-side. A
  `chars: 0` beacon is the "nothing to attach" toast: the clipboard carried
  neither bytes nor a path, and nothing was pasted BY DESIGN.
