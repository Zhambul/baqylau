...
- **A launched/resumed session's first message carried an unexpected image (a
  screenshot the user never attached), e.g. `say test[Image #1]`** *(fixed
  2026-07-23 — bare launch + clear-then-send)* — this is Claude Code's TUI
  reading the macOS clipboard at interactive startup WHEN GIVEN AN ARGV PROMPT
  and auto-attaching whatever image is on it to that first, auto-submitted
  message (cached to CC's own `image-cache/<sid>/N.png`). NOT a baqylau send: the
  `web-launch` row shows `attachments: 0` and there's no `web-upload`/`web-send`.
  The fix launches BARE (no `[prompt]` argv word — verify the `web-launch` argv
  carries only `--resume`/`--model`/`--effort`, never the prompt) and delivers the
  first prompt post-launch via clear-then-send, leaving a **`web-launch-prompt`**
  row. Tell that the fix ran: a `web-launch` with `deferred_prompt: true` + a
  following `web-launch-prompt` `ok: true`. Tell that the FIRST PROMPT NEVER
  LANDED (a different bug): `deferred_prompt: true` but the `web-launch-prompt`
  row is `ok: false` (`why: no window`/`modal open`/`send failed`) or absent
  (delivery thread never found the session — cross-check `web-launch-wake`
  `ok: false`). If a screenshot STILL rides a first message on a current build,
  the argv path regressed (a `[prompt]` word back in the `web-launch` argv) or a
  pre-fix server is running (restart `claude-dashboard.py`).
