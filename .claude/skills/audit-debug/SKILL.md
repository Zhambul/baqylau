...
- **No Telegram alert for a session left red/green on the dashboard** (docs/dashboard.md
  *Telegram alerts* — the deferred off-device notification): the alert fires only if the
  ...
  gets none); whether you reacted AT THE TERMINAL without moving the tab — a
  `notify-suppress` `state_files` row means the arm was dropped because you were typing
  there: `reason: dialog-activity` (a red `asking` arm — you edited the AskUserQuestion
  dialog) or `reason: terminal-input` (a green `done` arm — you typed a reply into the `❯`
  input box, `suggestion.typed` saw real non-faint text); and the dashboard actually
