...
- **Web STOP "did nothing" — the turn KEPT RUNNING (and the tab may have flipped
  green anyway)** *(unverified single-Esc miss, fixed 2026-07-24, session
  a16a181f)* — the STOP button is an INTERRUPT (`web-interrupt`), not the close.
  A single synthesized Escape via `kitten @ send-key` is only ~2/3 reliable and
  kitty reports no delivery, so a blind press can be accepted (`ok: true`) yet
  never reach the TUI — the turn runs to natural completion. The pre-fix tell:
  a `web-interrupt {ok:true, tab:thinking}` with NO `attempts`/`stopped` fields,
  an `escape-recheck` that flipped `thinking → awaiting-response` (masking it),
  yet `hook_events` `MessageDisplay` rows keep firing for tens of seconds AFTER
  the press and the real `Stop` (`claude-stop-fmt.py`) fires much later — the
  turn finished on its own, not on the Esc. On a current build the endpoint
  VERIFIES via the `out: N tok/s` output-rate footer and re-presses: a `web-interrupt
  stopped:false` (+ `errors` func `dashboard interrupt (not stopped)`, a 502) is
  the verify CORRECTLY reporting the miss (no masking escape-recheck fires); a
  recurrence with `stopped:true` but the turn still running = the spinner was
  out of the viewport when probed (get_text reads the scrolled-to viewport). A
  `web-interrupt` with NO `attempts`/`stopped` fields at all = a pre-fix server
  is still running (restart `claude-dashboard.py`).
