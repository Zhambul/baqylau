...
- **A `.md`/`.json` file rendered raw (or garbled) instead of pretty** — content
  render mode (markdown: `cat`/`head`/`tail` of a `.md`; JSON: `cat` of a `.json`;
  README § Command mirror pane). First confirm it was even engaged: the cmd-pre
  `hook_events` decision ends in `[md-render]`/`[json-render]`, and a `state_files`
  `render:<taskid>` `start` row exists (content `kind`) — if neither, `md_source`/
  `json_source` didn't match (piped/redirected/chained command, a non-allowlisted
  tool like `bat`/`glow`/`jq`, `head`/`tail` of a `.json` — JSON needs the whole
  file — or `CLAUDE_MIRROR_MD`/`_JSON=0`), so raw is expected. If engaged but the
