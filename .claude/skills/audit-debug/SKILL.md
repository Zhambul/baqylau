...
- **No Telegram alert for a session left red/green on the dashboard** (docs/dashboard.md
  *Telegram alerts* — the deferred off-device notification): the alert fires only if the
  ...
  in order: **the device push fired FIRST and you reacted before the escalation** (since
  2026-07-24 the model is device-first, Telegram-if-ignored: after the grace a `web-push`
  `action:send` goes to your most-recently-used device, and Telegram only ESCALATES
  ...
  saw real non-faint text) / `tab-focused` (the kitty tab was frontmost at send time) /
  `web-viewing` (a browser was viewing the session — a fresh `/api/presence` beat carrying
  the sid, or the legacy `/api/session/<sid>/viewing`); and the dashboard actually
  running (no open `kind='dashboard'` streams row = the notifier isn't polling at all).
  The reverse — an alert you DIDN'T want — is the per-session mute (the 🔕 toggle) or a
  lower `CLAUDE_DASH_NOTIFY_DELAY_S`.
...
- **No on-device (iPad/desktop) push notification** (docs/dashboard.md *Web push* — the
  ...
  device, read the **`notify-route`** row (since 2026-07-24) for that sid: it names the
  ...
  was freshest (the smallest `age_s`). Then the `action: send` rows (each carries the
  target `device`) confirm delivery. `legacy:true` in notify-route = untagged subs
  ...
  whole lifecycle is anchored by `notify-arm` (`phase:arm` on the transition, `phase:
  escalate` when the push arms the Telegram nudge).
...
- **Web STOP "did nothing" — the turn KEPT RUNNING** *(vim-mode single-Esc miss,
  ...
  field at all = a pre-fix server (restart `claude-dashboard.py`).
...
- **A web attachment/screenshot "didn't attach" or the message went out without
  ...
  dashboard one.
- **A pasted file came out as a bare NAME (or nothing) instead of its full
  path** *(Web attachments → Promise-only files, 2026-07-25)* — copying a file
  out of an IDE (IntelliJ's Project-view *Copy*) puts a **zero-byte file
  PROMISE** on the clipboard, not bytes. Three evolutionary layers, so date the
  evidence:
  1. A `web-upload` row with `ok: false, why: "empty file"` = the composer
     UPLOADED the husk (pre-fix). A NEW one means the client is running STALE
     JS — cross-check the same device's `boot` row — or the user really picked
     a 0-byte file, which the picker now refuses client-side.
  2. A `web-client` `attach.promise-paste` with `chars: 0` and no `resolved`
     key = the first fix, which let the browser's default paste through. It
     pasted nothing, because a file paste's text flavor is empty in Chrome (and
     is only the BARE NAME even when readable) — the path is never exposed to
     script.
  3. Current: the page POSTs the basenames to `/api/clipboard/files` and the
     SERVER reads the pasteboard. Pair the server's `web-clipboard` row
     (`{names, matched, paths}` — what was asked, what was found) with the
     browser's `attach.promise-paste` (`{n, resolved, chars}` — what actually
     landed in the box). `matched: 0` = the host's pasteboard did not hold
     those basenames: either the clipboard moved on between the copy and the
     paste, or the pasting device is NOT the host (a phone over the tunnel —
     the correlation guard doing its job), and the page fell back to the bare
     name. An `errors` row `dashboard clipboard (read failed)` = no pyobjc / no
     pasteboard on the host, which also degrades to the bare name.
  A `chars: 0` beacon in any layer is the "nothing to attach" toast: nothing
  was pasted BY DESIGN, not a swallowed failure.
...
- **Session vanished from the web dashboard AND its mirror toggles up EMPTY —
  ...
  `_live_windows` scan finds no live pane and demotes it to `live:false`
