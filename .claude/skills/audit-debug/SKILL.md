...
- **Quick-command button (compact / model / effort) "did nothing"** *(since
  ...
  `ok: true` on an idle tab with no effect = check the transcript for the slash
  command's record (the TUI parses it — a typo'd model alias errors in-chat,
  invisible to the audit by design: the TUI stays authoritative).
