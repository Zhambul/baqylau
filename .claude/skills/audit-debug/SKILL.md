...
- **A `.md` file rendered raw (or garbled) instead of pretty** — markdown mode
  (`cat`/`head`/`tail` of a `.md`; README § Command mirror pane). First confirm it
  was even engaged: the cmd-pre `hook_events` decision ends in `[md-render]`, and a
  `state_files` `md-render:<taskid>` `start` row exists — if neither, `md_source`
  didn't match (piped/redirected/chained command, or a non-allowlisted tool like
  `bat`/`glow`, or `CLAUDE_MIRROR_MD=0`), so raw is expected. If engaged but the
  block is missing/garbled: the `start` row's `wenmode` field says whether the
  parser was importable (`false` → it fell back to the `render.markdown()` subset,
  which is line-oriented — no tables/fenced blocks/nesting; install `wenmode`); and
  the `done` row's `blocks` count — **`blocks: 0`** (surfaced by `anomalies`) means
  the renderer produced nothing (a parse crash — check `errors` for `claude-stream`,
  or an empty source). A stray literal `#`/`**` in the mirror with NO `md-render`
  rows is just a normal verbatim stream, not a bug.
