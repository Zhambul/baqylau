...
- **Starting a session from the dashboard yanks macOS focus to kitty (the user
  wanted to stay in the browser)** *(guard added 2026-07-18)* — pull the
  `state_files` `web-launch-refocus` row that should follow the `web-launch`
  row (both log/path-empty; query `sql "SELECT * FROM state_files WHERE action
  LIKE 'web-launch%' ORDER BY id DESC LIMIT 10"`). No refocus row at all = the
  guard never armed: the frontend had no `app_id()`, the terminal was ALREADY
  frontmost at click time (then kitty keeping focus is correct behavior), or a
  pre-guard server is still running (restart `claude-dashboard.py`). `outcome:
  clean` while the user saw the steal = it landed outside the ~2s watch window
  or something other than the terminal app took focus. `bounced to <app>` =
  worked as designed (a brief kitty flash then back is the designed behavior,
  not a bug). `activate failed` = `open -b` broke — check the paired thread
  environment. The steal itself is arrangement-dependent kitty behavior
  (fullscreen/Spaces-layout-sensitive; fresh instances don't reproduce it) and
  do NOT "fix" it by adding `--keep-focus` to the launch: on a background
  kitty that flag CAUSES an unconditional steal (kitty "restores" focus via
  focus_os_window(raise=True) whenever no kitty OS window is focused —
  verified against plain-config kitty 0.45, docs/dashboard.md *The
  focus-bounce guard*).
