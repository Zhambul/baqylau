...
- **New-session form lost the half-typed FIRST PROMPT (closed the form and it
  came back blank), or shows a prompt that was already launched** → the draft
  is GLOBAL, not per-session: log/path-empty `ns-draft` `state_files` rows
  (`sql "SELECT * FROM state_files WHERE action='ns-draft' ORDER BY id DESC"`),
  content `{action, chars, seq}` (docs/dashboard.md *New-session draft*). A
  close with NO `write` row (or one with a smaller `chars` than what was typed)
  ⇒ the flush never landed — `closeNewSession`'s immediate save didn't reach
  the server (check the frontend-audit `web-client` rows for the page's
  ...
  the clear — compare its `seq` against the neighbouring `write`, the newest
  `seq` is what the box restores. Symmetrically, a draft that vanished while
  the form was OPEN on another device is the on-open reconcile fetch adopting a
  newer server draft — expected, and the `write` rows show whose was newer.
