...
- **A `.md`/`.json`/`.yml`/source file rendered raw (or garbled) instead of
  ...
  whole file, or `CLAUDE_MIRROR_MD`/`_JSON`/`_YAML`/`_CODE=0`), so raw is expected. If engaged but the
