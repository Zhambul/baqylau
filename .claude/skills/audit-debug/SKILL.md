...
- **A block shows in the terminal mirror but is missing from the web dashboard's
  ...
  *View modes*, since 2026-07-25) — in `default` the web stream collapses runs of
  ...
  check the op JSON for a `src` field. Stamped ops (`sub:<agent_id>` / `team:<agent_id>` / `codex:<label>`)
...
- **An agent's scoped mirror is EMPTY though the agent clearly worked** — almost
  ...
  a subagent/teammate the scope is `sub:<id>`/`team:<id>`, but a CODEX run is
  stamped `codex:<label>` while its agent id is the rollout basename, so
  `read/mirror.agent_scope` has to resolve the label off the run's row.
...
- **A Telegram message / iPad banner that never went away after I dealt with the session**
  ...
  - **The reverse — an alert vanished before I got to it**: check the `reason`. It should
    only ever be `tab-moved`/`session-ended`/`composing`. A glance (`tab-focused`,
    `web-viewing`) appearing here would be the RETRACT_REASONS/SILENT_REASONS tables having
    drifted back together — that is a real regression, not a tuning question: a look must
    cancel a PENDING alert and never delete a DELIVERED one.
