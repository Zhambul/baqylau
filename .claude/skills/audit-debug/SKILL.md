...
- **No on-device (iPad/desktop) push notification** (docs/dashboard.md *Web push* — the
  ...
  `CLAUDE_DASH_NOTIFY_WEBPUSH=0`. PRESENT but still no banner → look for the `action: send`
  rows: since 2026-07-24 the push is DEVICE-ROUTED — it goes only to the MOST-RECENTLY-USED
  device's subscription(s), not every sub (the `subscribe` row's `device`/`label` identify
  them; presence comes from the un-audited `/api/presence` beat). So a `send` to a
  DIFFERENT device than expected = that other device beat more recently (its dashboard was
  focused/visible later); "the wrong device buzzed" is a presence question, not a bug.
  NONE despite the arm firing = suppressed/muted upstream EXACTLY like Telegram (same
  ...
  rotated, every prior `subscribe` silently orphans and all sends `ok:false`.
