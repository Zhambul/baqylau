...
- **Clicking a Read/Update/Write line doesn't expand (or won't collapse)** — the
  ...
  pane is running a pre-feature renderer — toggle the mirror off/on). Expansion scrolled the pane to
  the wrong place: `errors` func `scroll_to (view toggle)` (the kitten
  scroll-window call failed — socket/anchor problem, not geometry). Expanded code
