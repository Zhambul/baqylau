...
- **Web rewind failed / picked the wrong checkpoint / left the session inside a
  ...
  `option` = the requested restore mode wasn't on the confirm menu (code
  restore at a no-code-change checkpoint); `confirm`/`close` = a menu
